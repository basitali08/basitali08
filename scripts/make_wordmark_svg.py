"""
Render "BASIT" as an EXTRUDED 3D wordmark rasterized to ASCII, and emit it as an
...
Modes:
  rock   -- oscillates +/-11 deg around the rest pose, forever (README default)
...
def find_font():
    """Pick a heavy face we can count on. Futura on macOS, Arial Bold on Windows,
    otherwise the env override."""
    if os.environ.get("WORDMARK_FONT"):
        return os.environ["WORDMARK_FONT"]
    candidates = [
    ...
    for c in candidates:
        if os.path.exists(c):
            return c
    # last resort: any ttf Pillow can open
    return "arialbd.ttf"
...
TEXT = os.environ.get("WORDMARK_TEXT", "BASIT")
...
def emit(frames, mode, out, dur, reveal):
    ...
    p.append(f'<text x="{canvas_w/2:.0f}" y="{TITLEBAR_H/2 + 4:.0f}" fill="{TITLE_TEXT}" '
             f'font-size="11.5" text-anchor="middle">basitali08@github: ~$ ./wordmark.sh --3d</text>')
